...
import { Accelerometer, Gyroscope } from 'react-native-sensors'; // Import the required sensor APIs

const App = () => {
  // Initialize accelerometer and gyroscope sensor observables
  const accelerometerObservable = new Accelerometer();
  const gyroscopeObservable = new Gyroscope();

  const [accelerationValue, setAccelerationValue] = useState({ x: 0, y: 0, z: 0 });
  const [gyroscopeValue, setGyroscopeValue] = useState({ x: 0, y: 0, z: 0 });

  // Subscribe to accelerometer data
  useEffect(() => {
    const subscription = accelerometerObservable.subscribe(({ x, y, z }) => {
      setAccelerationValue({ x, y, z });
    });
  ...
  useEffect(() => {
    const subscription = gyroscopeObservable.subscribe(({ x, y, z }) => {
      setGyroscopeValue({ x, y, z });
    });

    return () => subscription.unsubscribe(); // Clean up the subscription
  }, []);

  // Membership functions, rules, and defuzzification logic remain the same as in your original code
...
const accidentSeverity = weightedSum / totalWeight;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.text}>Acceleration X: {accelerationValue.x.toFixed(2)}</Text>
      <Text style={styles.text}>Acceleration Y: {accelerationValue.y.toFixed(2)}</Text>
      <Text style={styles.text}>Acceleration Z: {accelerationValue.z.toFixed(2)}</Text>
      <Text style={styles.text}>Gyroscope X: {gyroscopeValue.x.toFixed(2)}</Text>
      <Text style={styles.text}>Gyroscope Y: {gyroscopeValue.y.toFixed(2)}</Text>
      <Text style={styles.text}>Gyroscope Z: {gyroscopeValue.z.toFixed(2)}</Text>
